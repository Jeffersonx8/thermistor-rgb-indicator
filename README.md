...
## How it works
I wanted to make a smart sensor project myself, basically something that takes in data, prints it out, and does something with that data. This project will be changing the shade of red or blue depending on the temperature. The colder the thermistor is, the closer the shade will be to blue, and the warmer it is, the closer the shade will be to red. For the range, I used 20C - 35C. For demonstration, I will be using 27 celsius or 300 kelvin to show the conversion process. 300 kelvin was a leftover threshold value from my first version of the project, which was to make a red LED blink when it crosses a specific threshold value and make the light stay constant when it was below it. This is my final version of the project, which is using a RGB LED, therefore not needing a specific threshold to cross. In the code, there are some variables that are unused in my RGB version because they are necessary for my first red LED version that has been commented out. 

To change the shade of red/blue on the RGB LED, I used a thermistor to detect temperature changes and convert it into resistance. I used SunFounder's official documentation of my specific thermistor to find the formula for it, which is 

$$R_T = R_N \cdot e^{B(\frac{1}{T_K} - \frac{1}{T_N})}$$

And the constants were going to be 10k ohms (RN) at 25 celsius which is 298 kelvin (TN). I also used room temperature of 27C so I can comfortably see visual changes in the color of the LED by just blowing on the thermistor or putting my finger on it. Below is the before and after of me warming it up with my finger's body temperature and the after of putting a ice mold on it.

![Red LED](media/red-led.jpeg) ![Blue LED](media/blue-led.jpeg)

As for formula derivations, I had to convert temperature to KELVIN (not celsius) into "units", which is what arduino reads. And since the arduino is 10 bits, the max units is 1023, and the max voltage the arduino can output is 5 volts. Meaning each unit is approximately $5/1023 = .0049 volts$. My goal now is to get temperature to voltage conversion. I simply plugged in the temperature I wanted, 300 kelvin, into the $R_T$ formula above as $T_K$. B is 3950, which is the beta constant provided in the SunFounder's documentation. The resistance of the thermistor is around 8389.84 ohms at room temperature of 300 kelvin. I also needed to include one other fixed resistor with a value of 10k ohms to match the $R_N$ constant of the formula and to create a junction for my analogRead() to detect changing voltage. Now, using Ohms Law, I found the current across the system, which will be the same everywhere throughout because I connected my resistor and thermistor in series.
...
4) A simple error of the light just not blinking when I was testing the red LED version, it was because i didn't realize 1 equal sign (=) was different than 2 equal signs(==). Two equal signs is used for comparison, one changes values.
...
## Real vs Ideal
Before physically building it, I built it on Tinkercad Circuits. It was infinitely easier because instead of using a thermistor, I had another component called the TMP36. It had a simple linear formula for changing temperature directly into volts, and the documentation also had a easy to read linear graph of temperature vs voltage, unlike what I had to do by turning temperature into resistance, then into volts. It was also much easier to control the temperature in a simulation. Because of the difference of components, the relationship between temperature in the real world was actually inverse to the relationship I had predicted on Tinkercad. See bug #2 above.
